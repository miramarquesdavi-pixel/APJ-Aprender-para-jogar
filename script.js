...
async function generateQuestions(){
  const key=`${S[si][0]}:${li}`;
  const previous=state.history.filter(x=>x.key===key).map(x=>x.question).slice(-30);
  // Também evita repetir perguntas já criadas nesta sessão.
  const currentSession=qs.map(x=>x[0]).filter(Boolean);
  const blocked=[...new Set([...previous,...currentSession])].slice(-35);
...
async function start(i,l){
  ...
  try{
    ...
    // Garante diversidade dentro do próprio lote.
    const normalized=qs.map(x=>x[0].toLowerCase().replace(/[^a-z0-9à-ÿ]+/gi,' ').trim());
    if(new Set(normalized).size!==normalized.length) throw new Error('A IA repetiu uma questão no mesmo lote.');
  }catch(error){
    console.warn(error);
